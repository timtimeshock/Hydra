 ...
 *   ANALYZE (Opus) → DESIGN (Claude) → CRITIQUE (Opus) → REFINE (Claude) → TEST (optional)
 *
...
\`\`\`json
{
  ...
  "suggestedBase": "claude|opus|codex",
...
- **baseAgent**: Which physical agent to run on ("claude", "opus", or "codex")
...
- security-reviewer (opus): Security focus, OWASP methodology, severity ratings, remediation guidance. Scores: security=0.98, review=0.92, analysis=0.85.
- test-writer (codex): Test strategy, edge cases, coverage. Scores: testing=0.98, implementation=0.65.
- researcher (opus): Systematic exploration, hypothesis-driven, evidence-based. Scores: research=0.98, analysis=0.90.
...
Physical: claude (architect), opus (critique), codex (implementer)
Virtual: security-reviewer (opus), test-writer (codex), doc-generator (claude), researcher (opus), evolve-researcher (opus)
...
export async function runForgePipeline(description, codebaseCtx = null, opts = {}) {
  ...
  const analyzeResult = await executeAgent('opus', analyzePrompt, {
    timeoutMs,
    useStdin: true,
  ...
  const critiqueResult = await executeAgent('opus', critiquePrompt, {
    timeoutMs,
    useStdin: true,
    maxOutputBytes: 64 * 1024,
    hubCwd: process.cwd(),
...
export async function runForgeWizard(rl, description = '') {
  ...
  // Step 2: Analyze
  console.log('');
  console.log(`  ${ACCENT('\u25B6')} Phase 1/4: ${pc.bold('ANALYZE')} ${DIM('(Opus scanning codebase...)')}`);
  const profile = analyzeCodebase();
  ...
  const { spec, phases, session } = await runForgePipeline(intent, profile, {
    onPhase: (name, status, phaseData) => {
      ...
      if (status === 'running') {
        ...
        const agent = name === 'analyze' || name === 'critique' ? 'Opus' : 'Claude';
