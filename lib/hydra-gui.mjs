...
async function daemonHealthy(timeoutMs = 1_500) {
  try {
    ...
    return Boolean(body?.running ?? body?.ok ?? true);
  } catch {
    return false;
  }
}

/** Prevent parallel daemon spawn storms after reboot / crash. */
let ensureDaemonInFlight = null;
let lastEnsureAttemptAt = 0;

async function ensureDaemon() {
  if (await daemonHealthy()) return true;
  if (ensureDaemonInFlight) return ensureDaemonInFlight;

  const now = Date.now();
  if (now - lastEnsureAttemptAt < 2_500) return false;
  lastEnsureAttemptAt = now;

  ensureDaemonInFlight = (async () => {
    const project = resolveProject({ skipValidation: true });
    appendLog('Starting daemon…');
    const spawnLog = path.join(project.coordDir || path.join(HYDRA_ROOT, 'docs', 'coordination'), 'daemon-spawn.log');
    try { fs.mkdirSync(path.dirname(spawnLog), { recursive: true }); } catch { /* ignore */ }

    let child;
    try {
      const out = fs.openSync(spawnLog, 'a');
      child = spawnHydraNode(path.join(HYDRA_ROOT, 'lib', 'orchestrator-daemon.mjs'), ['start'], {
        ...
        env: {
          ...process.env,
          HYDRA_PROJECT: process.env.HYDRA_PROJECT || project.projectRoot,
      ...
      child.on('exit', (code, signal) => {
        if (code && code !== 0) {
          appendLog(`Daemon spawn exited early (code ${code}${signal ? `, ${signal}` : ''}) — see daemon-spawn.log`);
        }
      });
      child.on('error', (err) => {
        appendLog(`Daemon spawn error: ${err.message}`);
      });
    } catch (err) {
      appendLog(`Daemon spawn failed: ${err.message}`);
      return false;
    }

    // Cold boot after reboot: Node + first listen can exceed 8s
    for (let i = 0; i < 60; i++) {
      ...
      if (child.exitCode != null && child.exitCode !== 0) {
        appendLog(`Daemon failed to stay up (exit ${child.exitCode})`);
        return false;
      }
    }
    appendLog('Daemon se nespustil včas — GUI zkusí znovu samo.');
    return false;
  })();

  try {
    return await ensureDaemonInFlight;
  } finally {
    ensureDaemonInFlight = null;
  }
}

/** Non-blocking retry when snapshot sees daemon down. */
function kickEnsureDaemon() {
  if (ensureDaemonInFlight) return;
  ensureDaemon().catch(() => {});
}
...
async function handleApi(req, res, url) {
  ...
  if (req.method === 'GET' && route === '/api/snapshot') {
    ...
    if (!health?.running) kickEnsureDaemon();
  ...
  if (req.method === 'POST' && route === '/api/daemon/ensure') {
    const ok = await ensureDaemon();
    sendJson(res, 200, { ok, daemon: ok });
    return;
  }
...
export async function startGui({ open = true } = {}) {
  loadPersistedLog();
  const pageUrl = `http://${GUI_HOST}:${GUI_PORT}/?v=12`;
  const ok = await ensureDaemon();
  if (!ok) appendLog('Daemon se nespustil — GUI poběží a bude daemon znovu zkoušet.');
  if (isConciergeAvailable()) {
